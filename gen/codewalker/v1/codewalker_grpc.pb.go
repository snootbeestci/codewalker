...
func (c *codeWalkerClient) ListFileOrderers(ctx context.Context, in *ListFileOrderersRequest, opts ...grpc.CallOption) (*ListFileOrderersResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(ListFileOrderersResponse)
	err := c.cc.Invoke(ctx, CodeWalker_ListFileOrderers_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _CodeWalker_ListFileOrderers_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ListFileOrderersRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(CodeWalkerServer).ListFileOrderers(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: CodeWalker_ListFileOrderers_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(CodeWalkerServer).ListFileOrderers(ctx, req.(*ListFileOrderersRequest))
	}
	return interceptor(ctx, in, info, handler)
}
